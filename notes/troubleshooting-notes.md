...
- Preserve persistence definitions before removing them.
- Do not rely only on current system state.
- Task Scheduler logs can retain historical evidence.
- Process telemetry can show task-management tools such as `schtasks.exe`.
- A missing deletion event should be documented rather than invented.
- Unrelated Wazuh events should not be used as supporting evidence.
- The same task name, process, user, and time window should be correlated before drawing conclusions.
